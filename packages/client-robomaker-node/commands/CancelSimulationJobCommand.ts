...
export class CancelSimulationJobCommand implements __aws_sdk_types.Command<
    InputTypesUnion,
...
> {
    ...
    ): __aws_sdk_types.Handler<CancelSimulationJobInput, CancelSimulationJobOutput> {
        const {handler} = configuration;
        const stack = clientStack.concat(this.middlewareStack);

        const handlerExecutionContext: __aws_sdk_types.HandlerExecutionContext = {
            logger: {} as any,
            model: CancelSimulationJob
        };

        return stack.resolve(
            handler<CancelSimulationJobInput, CancelSimulationJobOutput>(handlerExecutionContext),
