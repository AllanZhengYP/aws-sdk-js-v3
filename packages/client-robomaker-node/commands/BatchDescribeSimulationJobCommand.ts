...
export class BatchDescribeSimulationJobCommand implements __aws_sdk_types.Command<
    InputTypesUnion,
...
> {
    ...
    ): __aws_sdk_types.Handler<BatchDescribeSimulationJobInput, BatchDescribeSimulationJobOutput> {
        const {handler} = configuration;
        const stack = clientStack.concat(this.middlewareStack);

        const handlerExecutionContext: __aws_sdk_types.HandlerExecutionContext = {
            logger: {} as any,
            model: BatchDescribeSimulationJob
        };

        return stack.resolve(
            handler<BatchDescribeSimulationJobInput, BatchDescribeSimulationJobOutput>(handlerExecutionContext),
