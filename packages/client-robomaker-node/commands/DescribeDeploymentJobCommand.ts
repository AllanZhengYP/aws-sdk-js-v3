...
export class DescribeDeploymentJobCommand implements __aws_sdk_types.Command<
    InputTypesUnion,
...
> {
    ...
    ): __aws_sdk_types.Handler<DescribeDeploymentJobInput, DescribeDeploymentJobOutput> {
        const {handler} = configuration;
        const stack = clientStack.concat(this.middlewareStack);

        const handlerExecutionContext: __aws_sdk_types.HandlerExecutionContext = {
            logger: {} as any,
            model: DescribeDeploymentJob
        };

        return stack.resolve(
            handler<DescribeDeploymentJobInput, DescribeDeploymentJobOutput>(handlerExecutionContext),
