...
export class ListSimulationJobsCommand implements __aws_sdk_types.Command<
    InputTypesUnion,
...
> {
    ...
    ): __aws_sdk_types.Handler<ListSimulationJobsInput, ListSimulationJobsOutput> {
        const {handler} = configuration;
        const stack = clientStack.concat(this.middlewareStack);

        const handlerExecutionContext: __aws_sdk_types.HandlerExecutionContext = {
            logger: {} as any,
            model: ListSimulationJobs
        };

        return stack.resolve(
            handler<ListSimulationJobsInput, ListSimulationJobsOutput>(handlerExecutionContext),
