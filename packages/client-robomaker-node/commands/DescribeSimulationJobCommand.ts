...
export class DescribeSimulationJobCommand implements __aws_sdk_types.Command<
    InputTypesUnion,
...
> {
    ...
    ): __aws_sdk_types.Handler<DescribeSimulationJobInput, DescribeSimulationJobOutput> {
        const {handler} = configuration;
        const stack = clientStack.concat(this.middlewareStack);

        const handlerExecutionContext: __aws_sdk_types.HandlerExecutionContext = {
            logger: {} as any,
            model: DescribeSimulationJob
        };

        return stack.resolve(
            handler<DescribeSimulationJobInput, DescribeSimulationJobOutput>(handlerExecutionContext),
