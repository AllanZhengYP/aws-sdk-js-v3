...
export class ListRobotApplicationsCommand implements __aws_sdk_types.Command<
    InputTypesUnion,
...
> {
    ...
    ): __aws_sdk_types.Handler<ListRobotApplicationsInput, ListRobotApplicationsOutput> {
        const {handler} = configuration;
        const stack = clientStack.concat(this.middlewareStack);

        const handlerExecutionContext: __aws_sdk_types.HandlerExecutionContext = {
            logger: {} as any,
            model: ListRobotApplications
        };

        return stack.resolve(
            handler<ListRobotApplicationsInput, ListRobotApplicationsOutput>(handlerExecutionContext),
