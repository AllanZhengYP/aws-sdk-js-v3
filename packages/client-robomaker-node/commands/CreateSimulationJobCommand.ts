...
export class CreateSimulationJobCommand implements __aws_sdk_types.Command<
    InputTypesUnion,
...
> {
    ...
    ): __aws_sdk_types.Handler<CreateSimulationJobInput, CreateSimulationJobOutput> {
        const {handler} = configuration;
        const stack = clientStack.concat(this.middlewareStack);

        const handlerExecutionContext: __aws_sdk_types.HandlerExecutionContext = {
            logger: {} as any,
            model: CreateSimulationJob
        };

        return stack.resolve(
            handler<CreateSimulationJobInput, CreateSimulationJobOutput>(handlerExecutionContext),
