...
export class ListDeploymentJobsCommand implements __aws_sdk_types.Command<
    InputTypesUnion,
...
> {
    ...
    ): __aws_sdk_types.Handler<ListDeploymentJobsInput, ListDeploymentJobsOutput> {
        const {handler} = configuration;
        const stack = clientStack.concat(this.middlewareStack);

        const handlerExecutionContext: __aws_sdk_types.HandlerExecutionContext = {
            logger: {} as any,
            model: ListDeploymentJobs
        };

        return stack.resolve(
            handler<ListDeploymentJobsInput, ListDeploymentJobsOutput>(handlerExecutionContext),
