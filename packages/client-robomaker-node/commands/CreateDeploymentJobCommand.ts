...
export class CreateDeploymentJobCommand implements __aws_sdk_types.Command<
    InputTypesUnion,
...
> {
    ...
    ): __aws_sdk_types.Handler<CreateDeploymentJobInput, CreateDeploymentJobOutput> {
        const {handler} = configuration;
        const stack = clientStack.concat(this.middlewareStack);

        const handlerExecutionContext: __aws_sdk_types.HandlerExecutionContext = {
            logger: {} as any,
            model: CreateDeploymentJob
        };

        return stack.resolve(
            handler<CreateDeploymentJobInput, CreateDeploymentJobOutput>(handlerExecutionContext),
