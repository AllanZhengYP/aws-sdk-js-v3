...
export class DeleteSimulationApplicationCommand implements __aws_sdk_types.Command<
    InputTypesUnion,
...
> {
    ...
    ): __aws_sdk_types.Handler<DeleteSimulationApplicationInput, DeleteSimulationApplicationOutput> {
        const {handler} = configuration;
        const stack = clientStack.concat(this.middlewareStack);

        const handlerExecutionContext: __aws_sdk_types.HandlerExecutionContext = {
            logger: {} as any,
            model: DeleteSimulationApplication
        };

        return stack.resolve(
            handler<DeleteSimulationApplicationInput, DeleteSimulationApplicationOutput>(handlerExecutionContext),
