...
export class ListSimulationApplicationsCommand implements __aws_sdk_types.Command<
    InputTypesUnion,
...
> {
    ...
    ): __aws_sdk_types.Handler<ListSimulationApplicationsInput, ListSimulationApplicationsOutput> {
        const {handler} = configuration;
        const stack = clientStack.concat(this.middlewareStack);

        const handlerExecutionContext: __aws_sdk_types.HandlerExecutionContext = {
            logger: {} as any,
            model: ListSimulationApplications
        };

        return stack.resolve(
            handler<ListSimulationApplicationsInput, ListSimulationApplicationsOutput>(handlerExecutionContext),
