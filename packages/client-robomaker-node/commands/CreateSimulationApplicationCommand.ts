...
export class CreateSimulationApplicationCommand implements __aws_sdk_types.Command<
    InputTypesUnion,
...
> {
    ...
    ): __aws_sdk_types.Handler<CreateSimulationApplicationInput, CreateSimulationApplicationOutput> {
        const {handler} = configuration;
        const stack = clientStack.concat(this.middlewareStack);

        const handlerExecutionContext: __aws_sdk_types.HandlerExecutionContext = {
            logger: {} as any,
            model: CreateSimulationApplication
        };

        return stack.resolve(
            handler<CreateSimulationApplicationInput, CreateSimulationApplicationOutput>(handlerExecutionContext),
