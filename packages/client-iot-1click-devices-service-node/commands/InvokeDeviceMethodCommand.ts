...
export class InvokeDeviceMethodCommand implements __aws_sdk_types.Command<
    InputTypesUnion,
...
> {
    ...
    ): __aws_sdk_types.Handler<InvokeDeviceMethodInput, InvokeDeviceMethodOutput> {
        const {handler} = configuration;
        const stack = clientStack.concat(this.middlewareStack);

        const handlerExecutionContext: __aws_sdk_types.HandlerExecutionContext = {
            logger: {} as any,
            model: InvokeDeviceMethod
        };

        return stack.resolve(
            handler<InvokeDeviceMethodInput, InvokeDeviceMethodOutput>(handlerExecutionContext),
