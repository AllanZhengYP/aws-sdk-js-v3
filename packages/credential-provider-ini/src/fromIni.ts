...
import { getDefaultAssumer } from "./getDefaultAssumer";
...
export interface FromIniInit extends SourceProfileInit {
   ...
   * credentials for the assumed role. If not set, a default assumer will be
   * used with an STS client created.
   *
  ...
  /**
   * If `roleAssumer` is not set, the default assumer will create the STS client
   * with the specified options. You should not rely on this option because it's
   * internal to SDK.
   *
   * @internal
   */
...
const resolveProfileData = async (
  ...
  if (isAssumeRoleProfile(data)) {
    ...
    const roleAssumer = options.roleAssumer || getDefaultAssumer({ ...(options?.stsOptions || {}), region });
    ...
    return roleAssumer(await sourceCreds, params);
  }
