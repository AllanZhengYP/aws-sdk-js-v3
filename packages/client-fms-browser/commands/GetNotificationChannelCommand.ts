...
export class GetNotificationChannelCommand implements __aws_sdk_types.Command<
    InputTypesUnion,
...
> {
    ...
    ): __aws_sdk_types.Handler<GetNotificationChannelInput, GetNotificationChannelOutput> {
        const {handler} = configuration;
        const stack = clientStack.concat(this.middlewareStack);

        const handlerExecutionContext: __aws_sdk_types.HandlerExecutionContext = {
            logger: {} as any,
            model: GetNotificationChannel
        };

        return stack.resolve(
            handler<GetNotificationChannelInput, GetNotificationChannelOutput>(handlerExecutionContext),
