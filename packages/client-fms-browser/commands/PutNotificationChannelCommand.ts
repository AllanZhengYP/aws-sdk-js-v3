...
export class PutNotificationChannelCommand implements __aws_sdk_types.Command<
    InputTypesUnion,
...
> {
    ...
    ): __aws_sdk_types.Handler<PutNotificationChannelInput, PutNotificationChannelOutput> {
        const {handler} = configuration;
        const stack = clientStack.concat(this.middlewareStack);

        const handlerExecutionContext: __aws_sdk_types.HandlerExecutionContext = {
            logger: {} as any,
            model: PutNotificationChannel
        };

        return stack.resolve(
            handler<PutNotificationChannelInput, PutNotificationChannelOutput>(handlerExecutionContext),
