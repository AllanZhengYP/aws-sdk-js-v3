...
export class UpdateEventSourceMappingCommand implements __aws_sdk_types.Command<
    InputTypesUnion,
...
> {
    ...
    ): __aws_sdk_types.Handler<UpdateEventSourceMappingInput, UpdateEventSourceMappingOutput> {
        const {handler} = configuration;
        const stack = clientStack.concat(this.middlewareStack);

        const handlerExecutionContext: __aws_sdk_types.HandlerExecutionContext = {
            logger: {} as any,
            model: UpdateEventSourceMapping
        };

        return stack.resolve(
            handler<UpdateEventSourceMappingInput, UpdateEventSourceMappingOutput>(handlerExecutionContext),
