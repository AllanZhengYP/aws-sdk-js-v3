...
export class DescribeLoggingOptionsCommand implements __aws_sdk_types.Command<
    InputTypesUnion,
...
> {
    ...
    ): __aws_sdk_types.Handler<DescribeLoggingOptionsInput, DescribeLoggingOptionsOutput> {
        const {handler} = configuration;
        const stack = clientStack.concat(this.middlewareStack);

        const handlerExecutionContext: __aws_sdk_types.HandlerExecutionContext = {
            logger: {} as any,
            model: DescribeLoggingOptions
        };

        return stack.resolve(
            handler<DescribeLoggingOptionsInput, DescribeLoggingOptionsOutput>(handlerExecutionContext),
