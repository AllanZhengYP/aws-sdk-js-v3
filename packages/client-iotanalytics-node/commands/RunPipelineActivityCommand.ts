...
export class RunPipelineActivityCommand implements __aws_sdk_types.Command<
    InputTypesUnion,
...
> {
    ...
    ): __aws_sdk_types.Handler<RunPipelineActivityInput, RunPipelineActivityOutput> {
        const {handler} = configuration;
        const stack = clientStack.concat(this.middlewareStack);

        const handlerExecutionContext: __aws_sdk_types.HandlerExecutionContext = {
            logger: {} as any,
            model: RunPipelineActivity
        };

        return stack.resolve(
            handler<RunPipelineActivityInput, RunPipelineActivityOutput>(handlerExecutionContext),
