...
export class CancelPipelineReprocessingCommand implements __aws_sdk_types.Command<
    InputTypesUnion,
...
> {
    ...
    ): __aws_sdk_types.Handler<CancelPipelineReprocessingInput, CancelPipelineReprocessingOutput> {
        const {handler} = configuration;
        const stack = clientStack.concat(this.middlewareStack);

        const handlerExecutionContext: __aws_sdk_types.HandlerExecutionContext = {
            logger: {} as any,
            model: CancelPipelineReprocessing
        };

        return stack.resolve(
            handler<CancelPipelineReprocessingInput, CancelPipelineReprocessingOutput>(handlerExecutionContext),
