...
export function modeledEndpointMiddleware<InputType extends object>(
    ...
    >(
        ...
        { model: { http: { requestUri } } }: HandlerExecutionContext
    ): BuildHandler<Input, Output, any> => async (
        { request, input, ...rest }: BuildHandlerArguments<Input, any>
    ): Promise<Output> => {
        const endpoint = urlParser(input[inputKey] as any);
        let [pathPattern] = requestUri.split('?');
        const pathMatch = request.path.match(new RegExp(
            pathPattern.split('/')
                .map(
                    pathEl => GREEDY_PATH_PARAMETER_PATTERN.test(pathEl)
                        ? '.+'
