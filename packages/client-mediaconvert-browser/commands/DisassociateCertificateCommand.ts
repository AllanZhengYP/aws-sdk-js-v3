...
export class DisassociateCertificateCommand implements __aws_sdk_types.Command<
    InputTypesUnion,
...
> {
    ...
    ): __aws_sdk_types.Handler<DisassociateCertificateInput, DisassociateCertificateOutput> {
        const {handler} = configuration;
        const stack = clientStack.concat(this.middlewareStack);

        const handlerExecutionContext: __aws_sdk_types.HandlerExecutionContext = {
            logger: {} as any,
            model: DisassociateCertificate
        };

        return stack.resolve(
            handler<DisassociateCertificateInput, DisassociateCertificateOutput>(handlerExecutionContext),
