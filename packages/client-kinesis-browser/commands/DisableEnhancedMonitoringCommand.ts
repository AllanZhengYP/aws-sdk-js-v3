...
> {
    ...
    ): __aws_sdk_types.Handler<DisableEnhancedMonitoringInput, DisableEnhancedMonitoringOutput> {
        const {handler} = configuration;
        const stack = clientStack.concat(this.middlewareStack);

        const handlerExecutionContext: __aws_sdk_types.HandlerExecutionContext<DisableEnhancedMonitoringInput, DisableEnhancedMonitoringOutput, any> = {
            logger: {} as any,
            model: DisableEnhancedMonitoring
        };

        return stack.resolve(
            handler<DisableEnhancedMonitoringInput, DisableEnhancedMonitoringOutput>(handlerExecutionContext),
