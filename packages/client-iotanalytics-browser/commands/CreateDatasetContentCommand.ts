...
export class CreateDatasetContentCommand implements __aws_sdk_types.Command<
    InputTypesUnion,
...
> {
    ...
    ): __aws_sdk_types.Handler<CreateDatasetContentInput, CreateDatasetContentOutput> {
        const {handler} = configuration;
        const stack = clientStack.concat(this.middlewareStack);

        const handlerExecutionContext: __aws_sdk_types.HandlerExecutionContext = {
            logger: {} as any,
            model: CreateDatasetContent
        };

        return stack.resolve(
            handler<CreateDatasetContentInput, CreateDatasetContentOutput>(handlerExecutionContext),
