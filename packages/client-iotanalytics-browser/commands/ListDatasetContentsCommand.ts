...
export class ListDatasetContentsCommand implements __aws_sdk_types.Command<
    InputTypesUnion,
...
> {
    ...
    ): __aws_sdk_types.Handler<ListDatasetContentsInput, ListDatasetContentsOutput> {
        const {handler} = configuration;
        const stack = clientStack.concat(this.middlewareStack);

        const handlerExecutionContext: __aws_sdk_types.HandlerExecutionContext = {
            logger: {} as any,
            model: ListDatasetContents
        };

        return stack.resolve(
            handler<ListDatasetContentsInput, ListDatasetContentsOutput>(handlerExecutionContext),
