...
export class StartPipelineReprocessingCommand implements __aws_sdk_types.Command<
    InputTypesUnion,
...
> {
    ...
    ): __aws_sdk_types.Handler<StartPipelineReprocessingInput, StartPipelineReprocessingOutput> {
        const {handler} = configuration;
        const stack = clientStack.concat(this.middlewareStack);

        const handlerExecutionContext: __aws_sdk_types.HandlerExecutionContext = {
            logger: {} as any,
            model: StartPipelineReprocessing
        };

        return stack.resolve(
            handler<StartPipelineReprocessingInput, StartPipelineReprocessingOutput>(handlerExecutionContext),
