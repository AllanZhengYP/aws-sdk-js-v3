...
export class StartTranscriptionJobCommand implements __aws_sdk_types.Command<
    InputTypesUnion,
...
> {
    ...
    ): __aws_sdk_types.Handler<StartTranscriptionJobInput, StartTranscriptionJobOutput> {
        const {handler} = configuration;
        const stack = clientStack.concat(this.middlewareStack);

        const handlerExecutionContext: __aws_sdk_types.HandlerExecutionContext = {
            logger: {} as any,
            model: StartTranscriptionJob
        };

        return stack.resolve(
            handler<StartTranscriptionJobInput, StartTranscriptionJobOutput>(handlerExecutionContext),
