...
import { fromUtf8, toUtf8 } from '@aws-js-sdk-v3-prerelease/util-utf8-universal';
    ...
    const testCases: Array<[string, Uint8Array, MessageHeaders]> = [
        ...
        [
            ...
            ]),
            {
                '🦄': {
                    type: 'long',
                    value: new Int64(Uint8Array.from([
        ...
        [
            ...
            Uint8Array.from([
                0x04, 0xf0, 0x9f, 0xa6, 0x84,
                0x06, 0x00, 0x04, 0xde, 0xad, 0xbe, 0xef,
                0x04, 0xf0, 0x9f, 0x8f, 0x87,
                0x00,
                0x04, 0xf0, 0x9f, 0x90, 0x8e,
                0x07, 0x00, 0x07, 0xe2, 0x98, 0x83, 0xf0, 0x9f, 0x92, 0xa9,
                0x04, 0xf0, 0x9f, 0x90, 0xb4,
                0x01,
            ]),
            {
                '🦄': {
    ...
    describe('#parse', () => {
        for (const [description, encoded, decoded] of testCases) {
            it(`should parse ${description}`, () => {
                expect(marshaller.parse(new DataView(encoded.buffer)))
                    .toEqual(decoded);
            });
        }
        ...
        it('should throw when unrecognized header types are encountered', () => {
            ...
            expect(() => marshaller.parse(new DataView(header.buffer)))
                .toThrowError('Unrecognized header type tag');
        });
    });
})
