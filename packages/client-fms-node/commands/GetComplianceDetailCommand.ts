...
export class GetComplianceDetailCommand implements __aws_sdk_types.Command<
    InputTypesUnion,
...
> {
    ...
    ): __aws_sdk_types.Handler<GetComplianceDetailInput, GetComplianceDetailOutput> {
        const {handler} = configuration;
        const stack = clientStack.concat(this.middlewareStack);

        const handlerExecutionContext: __aws_sdk_types.HandlerExecutionContext = {
            logger: {} as any,
            model: GetComplianceDetail
        };

        return stack.resolve(
            handler<GetComplianceDetailInput, GetComplianceDetailOutput>(handlerExecutionContext),
