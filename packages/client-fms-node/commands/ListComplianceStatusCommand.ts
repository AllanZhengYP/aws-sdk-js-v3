...
export class ListComplianceStatusCommand implements __aws_sdk_types.Command<
    InputTypesUnion,
...
> {
    ...
    ): __aws_sdk_types.Handler<ListComplianceStatusInput, ListComplianceStatusOutput> {
        const {handler} = configuration;
        const stack = clientStack.concat(this.middlewareStack);

        const handlerExecutionContext: __aws_sdk_types.HandlerExecutionContext = {
            logger: {} as any,
            model: ListComplianceStatus
        };

        return stack.resolve(
            handler<ListComplianceStatusInput, ListComplianceStatusOutput>(handlerExecutionContext),
