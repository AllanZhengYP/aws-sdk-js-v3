...
export class DisassociateAdminAccountCommand implements __aws_sdk_types.Command<
    InputTypesUnion,
...
> {
    ...
    ): __aws_sdk_types.Handler<DisassociateAdminAccountInput, DisassociateAdminAccountOutput> {
        const {handler} = configuration;
        const stack = clientStack.concat(this.middlewareStack);

        const handlerExecutionContext: __aws_sdk_types.HandlerExecutionContext = {
            logger: {} as any,
            model: DisassociateAdminAccount
        };

        return stack.resolve(
            handler<DisassociateAdminAccountInput, DisassociateAdminAccountOutput>(handlerExecutionContext),
