...
export class DeleteNotificationChannelCommand implements __aws_sdk_types.Command<
    InputTypesUnion,
...
> {
    ...
    ): __aws_sdk_types.Handler<DeleteNotificationChannelInput, DeleteNotificationChannelOutput> {
        const {handler} = configuration;
        const stack = clientStack.concat(this.middlewareStack);

        const handlerExecutionContext: __aws_sdk_types.HandlerExecutionContext = {
            logger: {} as any,
            model: DeleteNotificationChannel
        };

        return stack.resolve(
            handler<DeleteNotificationChannelInput, DeleteNotificationChannelOutput>(handlerExecutionContext),
