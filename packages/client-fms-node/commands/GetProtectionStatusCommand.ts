...
export class GetProtectionStatusCommand implements __aws_sdk_types.Command<
    InputTypesUnion,
...
> {
    ...
    ): __aws_sdk_types.Handler<GetProtectionStatusInput, GetProtectionStatusOutput> {
        const {handler} = configuration;
        const stack = clientStack.concat(this.middlewareStack);

        const handlerExecutionContext: __aws_sdk_types.HandlerExecutionContext = {
            logger: {} as any,
            model: GetProtectionStatus
        };

        return stack.resolve(
            handler<GetProtectionStatusInput, GetProtectionStatusOutput>(handlerExecutionContext),
