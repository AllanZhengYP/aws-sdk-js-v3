...
export class AcceptDirectConnectGatewayAssociationProposalCommand implements __aws_sdk_types.Command<
    InputTypesUnion,
...
> {
    ...
    constructor(readonly input: AcceptDirectConnectGatewayAssociationProposalInput) {}

    resolveMiddleware(
        clientStack: __aws_sdk_middleware_stack.MiddlewareStack<InputTypesUnion, OutputTypesUnion, _stream.Readable>,
        configuration: DirectConnectResolvedConfiguration
    ): __aws_sdk_types.Handler<AcceptDirectConnectGatewayAssociationProposalInput, AcceptDirectConnectGatewayAssociationProposalOutput> {
        const {handler} = configuration;
        const stack = clientStack.concat(this.middlewareStack);

        const handlerExecutionContext: __aws_sdk_types.HandlerExecutionContext = {
            logger: {} as any,
            model: AcceptDirectConnectGatewayAssociationProposal
        };

        return stack.resolve(
            handler<AcceptDirectConnectGatewayAssociationProposalInput, AcceptDirectConnectGatewayAssociationProposalOutput>(handlerExecutionContext),
