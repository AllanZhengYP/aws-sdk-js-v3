...
export class AssociateVirtualInterfaceCommand implements __aws_sdk_types.Command<
    InputTypesUnion,
...
> {
    ...
    ): __aws_sdk_types.Handler<AssociateVirtualInterfaceInput, AssociateVirtualInterfaceOutput> {
        const {handler} = configuration;
        const stack = clientStack.concat(this.middlewareStack);

        const handlerExecutionContext: __aws_sdk_types.HandlerExecutionContext = {
            logger: {} as any,
            model: AssociateVirtualInterface
        };

        return stack.resolve(
            handler<AssociateVirtualInterfaceInput, AssociateVirtualInterfaceOutput>(handlerExecutionContext),
