...
export class ConfirmPrivateVirtualInterfaceCommand implements __aws_sdk_types.Command<
    InputTypesUnion,
...
> {
    ...
    ): __aws_sdk_types.Handler<ConfirmPrivateVirtualInterfaceInput, ConfirmPrivateVirtualInterfaceOutput> {
        const {handler} = configuration;
        const stack = clientStack.concat(this.middlewareStack);

        const handlerExecutionContext: __aws_sdk_types.HandlerExecutionContext = {
            logger: {} as any,
            model: ConfirmPrivateVirtualInterface
        };

        return stack.resolve(
            handler<ConfirmPrivateVirtualInterfaceInput, ConfirmPrivateVirtualInterfaceOutput>(handlerExecutionContext),
