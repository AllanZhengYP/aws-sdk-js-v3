...
export class DeleteVirtualInterfaceCommand implements __aws_sdk_types.Command<
    InputTypesUnion,
...
> {
    ...
    ): __aws_sdk_types.Handler<DeleteVirtualInterfaceInput, DeleteVirtualInterfaceOutput> {
        const {handler} = configuration;
        const stack = clientStack.concat(this.middlewareStack);

        const handlerExecutionContext: __aws_sdk_types.HandlerExecutionContext = {
            logger: {} as any,
            model: DeleteVirtualInterface
        };

        return stack.resolve(
            handler<DeleteVirtualInterfaceInput, DeleteVirtualInterfaceOutput>(handlerExecutionContext),
