...
export class CreatePrivateVirtualInterfaceCommand implements __aws_sdk_types.Command<
    InputTypesUnion,
...
> {
    ...
    ): __aws_sdk_types.Handler<CreatePrivateVirtualInterfaceInput, CreatePrivateVirtualInterfaceOutput> {
        const {handler} = configuration;
        const stack = clientStack.concat(this.middlewareStack);

        const handlerExecutionContext: __aws_sdk_types.HandlerExecutionContext = {
            logger: {} as any,
            model: CreatePrivateVirtualInterface
        };

        return stack.resolve(
            handler<CreatePrivateVirtualInterfaceInput, CreatePrivateVirtualInterfaceOutput>(handlerExecutionContext),
