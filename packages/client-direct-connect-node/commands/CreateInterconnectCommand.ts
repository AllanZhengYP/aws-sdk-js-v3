...
export class CreateInterconnectCommand implements __aws_sdk_types.Command<
    InputTypesUnion,
...
> {
    ...
    ): __aws_sdk_types.Handler<CreateInterconnectInput, CreateInterconnectOutput> {
        const {handler} = configuration;
        const stack = clientStack.concat(this.middlewareStack);

        const handlerExecutionContext: __aws_sdk_types.HandlerExecutionContext = {
            logger: {} as any,
            model: CreateInterconnect
        };

        return stack.resolve(
            handler<CreateInterconnectInput, CreateInterconnectOutput>(handlerExecutionContext),
