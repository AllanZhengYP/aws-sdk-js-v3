...
export class DisassociateConnectionFromLagCommand implements __aws_sdk_types.Command<
    InputTypesUnion,
...
> {
    ...
    ): __aws_sdk_types.Handler<DisassociateConnectionFromLagInput, DisassociateConnectionFromLagOutput> {
        const {handler} = configuration;
        const stack = clientStack.concat(this.middlewareStack);

        const handlerExecutionContext: __aws_sdk_types.HandlerExecutionContext = {
            logger: {} as any,
            model: DisassociateConnectionFromLag
        };

        return stack.resolve(
            handler<DisassociateConnectionFromLagInput, DisassociateConnectionFromLagOutput>(handlerExecutionContext),
