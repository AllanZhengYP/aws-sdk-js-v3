...
export class ConfirmPublicVirtualInterfaceCommand implements __aws_sdk_types.Command<
    InputTypesUnion,
...
> {
    ...
    ): __aws_sdk_types.Handler<ConfirmPublicVirtualInterfaceInput, ConfirmPublicVirtualInterfaceOutput> {
        const {handler} = configuration;
        const stack = clientStack.concat(this.middlewareStack);

        const handlerExecutionContext: __aws_sdk_types.HandlerExecutionContext = {
            logger: {} as any,
            model: ConfirmPublicVirtualInterface
        };

        return stack.resolve(
            handler<ConfirmPublicVirtualInterfaceInput, ConfirmPublicVirtualInterfaceOutput>(handlerExecutionContext),
