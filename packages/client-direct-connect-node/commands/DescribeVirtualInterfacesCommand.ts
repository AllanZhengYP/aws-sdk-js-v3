...
export class DescribeVirtualInterfacesCommand implements __aws_sdk_types.Command<
    InputTypesUnion,
...
> {
    ...
    ): __aws_sdk_types.Handler<DescribeVirtualInterfacesInput, DescribeVirtualInterfacesOutput> {
        const {handler} = configuration;
        const stack = clientStack.concat(this.middlewareStack);

        const handlerExecutionContext: __aws_sdk_types.HandlerExecutionContext = {
            logger: {} as any,
            model: DescribeVirtualInterfaces
        };

        return stack.resolve(
            handler<DescribeVirtualInterfacesInput, DescribeVirtualInterfacesOutput>(handlerExecutionContext),
