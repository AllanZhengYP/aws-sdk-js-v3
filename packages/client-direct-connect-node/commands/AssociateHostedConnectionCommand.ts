...
export class AssociateHostedConnectionCommand implements __aws_sdk_types.Command<
    InputTypesUnion,
...
> {
    ...
    ): __aws_sdk_types.Handler<AssociateHostedConnectionInput, AssociateHostedConnectionOutput> {
        const {handler} = configuration;
        const stack = clientStack.concat(this.middlewareStack);

        const handlerExecutionContext: __aws_sdk_types.HandlerExecutionContext = {
            logger: {} as any,
            model: AssociateHostedConnection
        };

        return stack.resolve(
            handler<AssociateHostedConnectionInput, AssociateHostedConnectionOutput>(handlerExecutionContext),
