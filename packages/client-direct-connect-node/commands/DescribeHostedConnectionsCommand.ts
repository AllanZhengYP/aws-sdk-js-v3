...
export class DescribeHostedConnectionsCommand implements __aws_sdk_types.Command<
    InputTypesUnion,
...
> {
    ...
    ): __aws_sdk_types.Handler<DescribeHostedConnectionsInput, DescribeHostedConnectionsOutput> {
        const {handler} = configuration;
        const stack = clientStack.concat(this.middlewareStack);

        const handlerExecutionContext: __aws_sdk_types.HandlerExecutionContext = {
            logger: {} as any,
            model: DescribeHostedConnections
        };

        return stack.resolve(
            handler<DescribeHostedConnectionsInput, DescribeHostedConnectionsOutput>(handlerExecutionContext),
