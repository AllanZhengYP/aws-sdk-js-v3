...
export class AssociateConnectionWithLagCommand implements __aws_sdk_types.Command<
    InputTypesUnion,
...
> {
    ...
    ): __aws_sdk_types.Handler<AssociateConnectionWithLagInput, AssociateConnectionWithLagOutput> {
        const {handler} = configuration;
        const stack = clientStack.concat(this.middlewareStack);

        const handlerExecutionContext: __aws_sdk_types.HandlerExecutionContext = {
            logger: {} as any,
            model: AssociateConnectionWithLag
        };

        return stack.resolve(
            handler<AssociateConnectionWithLagInput, AssociateConnectionWithLagOutput>(handlerExecutionContext),
