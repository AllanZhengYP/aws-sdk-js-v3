...
export class ClaimDevicesByClaimCodeCommand implements __aws_sdk_types.Command<
    InputTypesUnion,
...
> {
    ...
    ): __aws_sdk_types.Handler<ClaimDevicesByClaimCodeInput, ClaimDevicesByClaimCodeOutput> {
        const {handler} = configuration;
        const stack = clientStack.concat(this.middlewareStack);

        const handlerExecutionContext: __aws_sdk_types.HandlerExecutionContext = {
            logger: {} as any,
            model: ClaimDevicesByClaimCode
        };

        return stack.resolve(
            handler<ClaimDevicesByClaimCodeInput, ClaimDevicesByClaimCodeOutput>(handlerExecutionContext),
