...
export class InitiateDeviceClaimCommand implements __aws_sdk_types.Command<
    InputTypesUnion,
...
> {
    ...
    ): __aws_sdk_types.Handler<InitiateDeviceClaimInput, InitiateDeviceClaimOutput> {
        const {handler} = configuration;
        const stack = clientStack.concat(this.middlewareStack);

        const handlerExecutionContext: __aws_sdk_types.HandlerExecutionContext = {
            logger: {} as any,
            model: InitiateDeviceClaim
        };

        return stack.resolve(
            handler<InitiateDeviceClaimInput, InitiateDeviceClaimOutput>(handlerExecutionContext),
