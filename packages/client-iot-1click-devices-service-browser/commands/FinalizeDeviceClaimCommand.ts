...
export class FinalizeDeviceClaimCommand implements __aws_sdk_types.Command<
    InputTypesUnion,
...
> {
    ...
    ): __aws_sdk_types.Handler<FinalizeDeviceClaimInput, FinalizeDeviceClaimOutput> {
        const {handler} = configuration;
        const stack = clientStack.concat(this.middlewareStack);

        const handlerExecutionContext: __aws_sdk_types.HandlerExecutionContext = {
            logger: {} as any,
            model: FinalizeDeviceClaim
        };

        return stack.resolve(
            handler<FinalizeDeviceClaimInput, FinalizeDeviceClaimOutput>(handlerExecutionContext),
