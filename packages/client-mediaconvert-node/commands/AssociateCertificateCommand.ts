...
export class AssociateCertificateCommand implements __aws_sdk_types.Command<
    InputTypesUnion,
...
> {
    ...
    ): __aws_sdk_types.Handler<AssociateCertificateInput, AssociateCertificateOutput> {
        const {handler} = configuration;
        const stack = clientStack.concat(this.middlewareStack);

        const handlerExecutionContext: __aws_sdk_types.HandlerExecutionContext = {
            logger: {} as any,
            model: AssociateCertificate
        };

        return stack.resolve(
            handler<AssociateCertificateInput, AssociateCertificateOutput>(handlerExecutionContext),
