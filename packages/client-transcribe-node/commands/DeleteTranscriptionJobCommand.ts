...
export class DeleteTranscriptionJobCommand implements __aws_sdk_types.Command<
    InputTypesUnion,
...
> {
    ...
    ): __aws_sdk_types.Handler<DeleteTranscriptionJobInput, DeleteTranscriptionJobOutput> {
        const {handler} = configuration;
        const stack = clientStack.concat(this.middlewareStack);

        const handlerExecutionContext: __aws_sdk_types.HandlerExecutionContext = {
            logger: {} as any,
            model: DeleteTranscriptionJob
        };

        return stack.resolve(
            handler<DeleteTranscriptionJobInput, DeleteTranscriptionJobOutput>(handlerExecutionContext),
