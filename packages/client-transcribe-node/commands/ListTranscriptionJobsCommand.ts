...
export class ListTranscriptionJobsCommand implements __aws_sdk_types.Command<
    InputTypesUnion,
...
> {
    ...
    ): __aws_sdk_types.Handler<ListTranscriptionJobsInput, ListTranscriptionJobsOutput> {
        const {handler} = configuration;
        const stack = clientStack.concat(this.middlewareStack);

        const handlerExecutionContext: __aws_sdk_types.HandlerExecutionContext = {
            logger: {} as any,
            model: ListTranscriptionJobs
        };

        return stack.resolve(
            handler<ListTranscriptionJobsInput, ListTranscriptionJobsOutput>(handlerExecutionContext),
