...
export class CreateDirectConnectGatewayCommand implements __aws_sdk_types.Command<
    InputTypesUnion,
...
> {
    ...
    ): __aws_sdk_types.Handler<CreateDirectConnectGatewayInput, CreateDirectConnectGatewayOutput> {
        const {handler} = configuration;
        const stack = clientStack.concat(this.middlewareStack);

        const handlerExecutionContext: __aws_sdk_types.HandlerExecutionContext = {
            logger: {} as any,
            model: CreateDirectConnectGateway
        };

        return stack.resolve(
            handler<CreateDirectConnectGatewayInput, CreateDirectConnectGatewayOutput>(handlerExecutionContext),
