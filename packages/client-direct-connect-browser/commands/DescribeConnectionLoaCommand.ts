...
export class DescribeConnectionLoaCommand implements __aws_sdk_types.Command<
    InputTypesUnion,
...
> {
    ...
    ): __aws_sdk_types.Handler<DescribeConnectionLoaInput, DescribeConnectionLoaOutput> {
        const {handler} = configuration;
        const stack = clientStack.concat(this.middlewareStack);

        const handlerExecutionContext: __aws_sdk_types.HandlerExecutionContext = {
            logger: {} as any,
            model: DescribeConnectionLoa
        };

        return stack.resolve(
            handler<DescribeConnectionLoaInput, DescribeConnectionLoaOutput>(handlerExecutionContext),
