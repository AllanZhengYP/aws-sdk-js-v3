...
export class UpdateVirtualInterfaceAttributesCommand implements __aws_sdk_types.Command<
    InputTypesUnion,
...
> {
    ...
    ): __aws_sdk_types.Handler<UpdateVirtualInterfaceAttributesInput, UpdateVirtualInterfaceAttributesOutput> {
        const {handler} = configuration;
        const stack = clientStack.concat(this.middlewareStack);

        const handlerExecutionContext: __aws_sdk_types.HandlerExecutionContext = {
            logger: {} as any,
            model: UpdateVirtualInterfaceAttributes
        };

        return stack.resolve(
            handler<UpdateVirtualInterfaceAttributesInput, UpdateVirtualInterfaceAttributesOutput>(handlerExecutionContext),
