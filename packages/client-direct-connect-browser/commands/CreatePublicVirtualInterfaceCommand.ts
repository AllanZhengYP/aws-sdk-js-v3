...
export class CreatePublicVirtualInterfaceCommand implements __aws_sdk_types.Command<
    InputTypesUnion,
...
> {
    ...
    ): __aws_sdk_types.Handler<CreatePublicVirtualInterfaceInput, CreatePublicVirtualInterfaceOutput> {
        const {handler} = configuration;
        const stack = clientStack.concat(this.middlewareStack);

        const handlerExecutionContext: __aws_sdk_types.HandlerExecutionContext = {
            logger: {} as any,
            model: CreatePublicVirtualInterface
        };

        return stack.resolve(
            handler<CreatePublicVirtualInterfaceInput, CreatePublicVirtualInterfaceOutput>(handlerExecutionContext),
