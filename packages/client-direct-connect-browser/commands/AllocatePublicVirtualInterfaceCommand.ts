...
export class AllocatePublicVirtualInterfaceCommand implements __aws_sdk_types.Command<
    InputTypesUnion,
...
> {
    ...
    ): __aws_sdk_types.Handler<AllocatePublicVirtualInterfaceInput, AllocatePublicVirtualInterfaceOutput> {
        const {handler} = configuration;
        const stack = clientStack.concat(this.middlewareStack);

        const handlerExecutionContext: __aws_sdk_types.HandlerExecutionContext = {
            logger: {} as any,
            model: AllocatePublicVirtualInterface
        };

        return stack.resolve(
            handler<AllocatePublicVirtualInterfaceInput, AllocatePublicVirtualInterfaceOutput>(handlerExecutionContext),
