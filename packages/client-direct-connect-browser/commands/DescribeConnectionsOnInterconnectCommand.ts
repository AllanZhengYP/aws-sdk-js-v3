...
export class DescribeConnectionsOnInterconnectCommand implements __aws_sdk_types.Command<
    InputTypesUnion,
...
> {
    ...
    ): __aws_sdk_types.Handler<DescribeConnectionsOnInterconnectInput, DescribeConnectionsOnInterconnectOutput> {
        const {handler} = configuration;
        const stack = clientStack.concat(this.middlewareStack);

        const handlerExecutionContext: __aws_sdk_types.HandlerExecutionContext = {
            logger: {} as any,
            model: DescribeConnectionsOnInterconnect
        };

        return stack.resolve(
            handler<DescribeConnectionsOnInterconnectInput, DescribeConnectionsOnInterconnectOutput>(handlerExecutionContext),
