...
export class DescribeConnectionsCommand implements __aws_sdk_types.Command<
    InputTypesUnion,
...
> {
    ...
    ): __aws_sdk_types.Handler<DescribeConnectionsInput, DescribeConnectionsOutput> {
        const {handler} = configuration;
        const stack = clientStack.concat(this.middlewareStack);

        const handlerExecutionContext: __aws_sdk_types.HandlerExecutionContext = {
            logger: {} as any,
            model: DescribeConnections
        };

        return stack.resolve(
            handler<DescribeConnectionsInput, DescribeConnectionsOutput>(handlerExecutionContext),
