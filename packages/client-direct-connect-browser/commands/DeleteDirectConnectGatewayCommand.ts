...
export class DeleteDirectConnectGatewayCommand implements __aws_sdk_types.Command<
    InputTypesUnion,
...
> {
    ...
    ): __aws_sdk_types.Handler<DeleteDirectConnectGatewayInput, DeleteDirectConnectGatewayOutput> {
        const {handler} = configuration;
        const stack = clientStack.concat(this.middlewareStack);

        const handlerExecutionContext: __aws_sdk_types.HandlerExecutionContext = {
            logger: {} as any,
            model: DeleteDirectConnectGateway
        };

        return stack.resolve(
            handler<DeleteDirectConnectGatewayInput, DeleteDirectConnectGatewayOutput>(handlerExecutionContext),
