...
export class DescribeVirtualGatewaysCommand implements __aws_sdk_types.Command<
    InputTypesUnion,
...
> {
    ...
    ): __aws_sdk_types.Handler<DescribeVirtualGatewaysInput, DescribeVirtualGatewaysOutput> {
        const {handler} = configuration;
        const stack = clientStack.concat(this.middlewareStack);

        const handlerExecutionContext: __aws_sdk_types.HandlerExecutionContext = {
            logger: {} as any,
            model: DescribeVirtualGateways
        };

        return stack.resolve(
            handler<DescribeVirtualGatewaysInput, DescribeVirtualGatewaysOutput>(handlerExecutionContext),
