...
export class AllocatePrivateVirtualInterfaceCommand implements __aws_sdk_types.Command<
    InputTypesUnion,
...
> {
    ...
    ): __aws_sdk_types.Handler<AllocatePrivateVirtualInterfaceInput, AllocatePrivateVirtualInterfaceOutput> {
        const {handler} = configuration;
        const stack = clientStack.concat(this.middlewareStack);

        const handlerExecutionContext: __aws_sdk_types.HandlerExecutionContext = {
            logger: {} as any,
            model: AllocatePrivateVirtualInterface
        };

        return stack.resolve(
            handler<AllocatePrivateVirtualInterfaceInput, AllocatePrivateVirtualInterfaceOutput>(handlerExecutionContext),
