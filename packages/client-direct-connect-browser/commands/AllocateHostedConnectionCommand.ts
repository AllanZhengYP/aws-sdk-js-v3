...
export class AllocateHostedConnectionCommand implements __aws_sdk_types.Command<
    InputTypesUnion,
...
> {
    ...
    ): __aws_sdk_types.Handler<AllocateHostedConnectionInput, AllocateHostedConnectionOutput> {
        const {handler} = configuration;
        const stack = clientStack.concat(this.middlewareStack);

        const handlerExecutionContext: __aws_sdk_types.HandlerExecutionContext = {
            logger: {} as any,
            model: AllocateHostedConnection
        };

        return stack.resolve(
            handler<AllocateHostedConnectionInput, AllocateHostedConnectionOutput>(handlerExecutionContext),
