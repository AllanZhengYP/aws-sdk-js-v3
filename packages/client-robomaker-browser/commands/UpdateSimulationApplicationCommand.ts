...
export class UpdateSimulationApplicationCommand implements __aws_sdk_types.Command<
    InputTypesUnion,
...
> {
    ...
    ): __aws_sdk_types.Handler<UpdateSimulationApplicationInput, UpdateSimulationApplicationOutput> {
        const {handler} = configuration;
        const stack = clientStack.concat(this.middlewareStack);

        const handlerExecutionContext: __aws_sdk_types.HandlerExecutionContext = {
            logger: {} as any,
            model: UpdateSimulationApplication
        };

        return stack.resolve(
            handler<UpdateSimulationApplicationInput, UpdateSimulationApplicationOutput>(handlerExecutionContext),
