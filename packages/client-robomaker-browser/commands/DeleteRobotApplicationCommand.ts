...
export class DeleteRobotApplicationCommand implements __aws_sdk_types.Command<
    InputTypesUnion,
...
> {
    ...
    ): __aws_sdk_types.Handler<DeleteRobotApplicationInput, DeleteRobotApplicationOutput> {
        const {handler} = configuration;
        const stack = clientStack.concat(this.middlewareStack);

        const handlerExecutionContext: __aws_sdk_types.HandlerExecutionContext = {
            logger: {} as any,
            model: DeleteRobotApplication
        };

        return stack.resolve(
            handler<DeleteRobotApplicationInput, DeleteRobotApplicationOutput>(handlerExecutionContext),
