...
export class UpdateRobotApplicationCommand implements __aws_sdk_types.Command<
    InputTypesUnion,
...
> {
    ...
    ): __aws_sdk_types.Handler<UpdateRobotApplicationInput, UpdateRobotApplicationOutput> {
        const {handler} = configuration;
        const stack = clientStack.concat(this.middlewareStack);

        const handlerExecutionContext: __aws_sdk_types.HandlerExecutionContext = {
            logger: {} as any,
            model: UpdateRobotApplication
        };

        return stack.resolve(
            handler<UpdateRobotApplicationInput, UpdateRobotApplicationOutput>(handlerExecutionContext),
