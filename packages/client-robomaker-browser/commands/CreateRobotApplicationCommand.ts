...
export class CreateRobotApplicationCommand implements __aws_sdk_types.Command<
    InputTypesUnion,
...
> {
    ...
    ): __aws_sdk_types.Handler<CreateRobotApplicationInput, CreateRobotApplicationOutput> {
        const {handler} = configuration;
        const stack = clientStack.concat(this.middlewareStack);

        const handlerExecutionContext: __aws_sdk_types.HandlerExecutionContext = {
            logger: {} as any,
            model: CreateRobotApplication
        };

        return stack.resolve(
            handler<CreateRobotApplicationInput, CreateRobotApplicationOutput>(handlerExecutionContext),
