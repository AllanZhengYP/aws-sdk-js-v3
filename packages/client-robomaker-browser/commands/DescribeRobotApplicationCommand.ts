...
export class DescribeRobotApplicationCommand implements __aws_sdk_types.Command<
    InputTypesUnion,
...
> {
    ...
    ): __aws_sdk_types.Handler<DescribeRobotApplicationInput, DescribeRobotApplicationOutput> {
        const {handler} = configuration;
        const stack = clientStack.concat(this.middlewareStack);

        const handlerExecutionContext: __aws_sdk_types.HandlerExecutionContext = {
            logger: {} as any,
            model: DescribeRobotApplication
        };

        return stack.resolve(
            handler<DescribeRobotApplicationInput, DescribeRobotApplicationOutput>(handlerExecutionContext),
