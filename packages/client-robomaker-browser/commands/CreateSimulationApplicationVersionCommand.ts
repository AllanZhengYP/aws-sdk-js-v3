...
export class CreateSimulationApplicationVersionCommand implements __aws_sdk_types.Command<
    InputTypesUnion,
...
> {
    ...
    ): __aws_sdk_types.Handler<CreateSimulationApplicationVersionInput, CreateSimulationApplicationVersionOutput> {
        const {handler} = configuration;
        const stack = clientStack.concat(this.middlewareStack);

        const handlerExecutionContext: __aws_sdk_types.HandlerExecutionContext = {
            logger: {} as any,
            model: CreateSimulationApplicationVersion
        };

        return stack.resolve(
            handler<CreateSimulationApplicationVersionInput, CreateSimulationApplicationVersionOutput>(handlerExecutionContext),
