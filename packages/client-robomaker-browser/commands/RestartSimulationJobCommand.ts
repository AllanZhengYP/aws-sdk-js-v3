...
export class RestartSimulationJobCommand implements __aws_sdk_types.Command<
    InputTypesUnion,
...
> {
    ...
    ): __aws_sdk_types.Handler<RestartSimulationJobInput, RestartSimulationJobOutput> {
        const {handler} = configuration;
        const stack = clientStack.concat(this.middlewareStack);

        const handlerExecutionContext: __aws_sdk_types.HandlerExecutionContext = {
            logger: {} as any,
            model: RestartSimulationJob
        };

        return stack.resolve(
            handler<RestartSimulationJobInput, RestartSimulationJobOutput>(handlerExecutionContext),
