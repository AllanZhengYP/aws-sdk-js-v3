...
import {
  NullAndEmptyHeadersCommandInput,
  NullAndEmptyHeadersCommandOutput,
} from "../commands/NullAndEmptyHeadersCommand";
import {
...
export async function serializeAws_restJson1_1NullAndEmptyHeadersCommand(
  input: NullAndEmptyHeadersCommandInput,
  context: __SerdeContext
): Promise<__HttpRequest> {
  const headers: any = {};
  headers['Content-Type'] = "";
  ...
  let resolvedPath = "/NullAndEmptyHeaders";
  let body: any;
  return new __HttpRequest({
    ...context.endpoint,
    protocol: "https",
...
export async function deserializeAws_restJson1_1InputAndOutputWithHeadersCommand(
  ...
  if (output.headers["x-timestamplist"] !== undefined) {
    contents.headerTimestampList = (output.headers['x-timestamplist'] || "").split(',').map(_entry => new Date(_entry.trim()));
  }
...
export async function deserializeAws_restJson1_1NullAndEmptyHeadersCommand(
  output: __HttpResponse,
  context: __SerdeContext
): Promise<NullAndEmptyHeadersCommandOutput> {
  if (output.statusCode !== 200 && output.statusCode >= 400) {
    return deserializeAws_restJson1_1NullAndEmptyHeadersCommandError(output, context);
  }
  const contents: NullAndEmptyHeadersCommandOutput = {
    $metadata: deserializeMetadata(output),
...
async function deserializeAws_restJson1_1NullAndEmptyHeadersCommandError(
  output: __HttpResponse,
  context: __SerdeContext,
): Promise<NullAndEmptyHeadersCommandOutput> {
  const parsedOutput: any = {
